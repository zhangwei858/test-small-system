...
const api = {
  ...
  getAllExamHistory: () => request('GET', '/exams/history/all'),
  getExamDetails: (examId) => request('GET', `/exams/${examId}/details`),
  login: (username) => request('POST', '/users/login', { username }),
  registerExaminer: (username, allowedGrades) => request('POST', '/users/register', { username, allowedGrades }),
  ...
  getPrizes: () => request('GET', '/lottery/prizes'),
  ...
  getDynamicProbabilities: (questionCount) => request('GET', `/prices/probability/dynamic?question_count=${questionCount}`),
