...
async def get_wrong_answers(user_name: str, page: int = 1, limit: int = 10, db: AsyncSession = Depends(get_db)):
    result = await wrong_answer_service.get_wrong_answers(db, user_name, page, limit)
    return ApiResponse(data=result["data"], **{"total": result["total"], "page": result["page"], "limit": result["limit"]})


@router.get("/user/{user_name}/count")
async def get_wrong_answer_count(user_name: str, db: AsyncSession = Depends(get_db)):
    count = await wrong_answer_service.get_wrong_answer_count(db, user_name)
    return ApiResponse(data={"count": count})


@router.delete("/{answer_id}")
async def delete_wrong_answer(answer_id: int, db: AsyncSession = Depends(get_db)):
...
async def clear_wrong_answers(user_name: str, db: AsyncSession = Depends(get_db)):
    await wrong_answer_service.clear_wrong_answers(db, user_name)
    return ApiResponse(message="清空错题本成功")


@router.post("/practice")
async def practice_wrong_answers(data: PracticeRequest, db: AsyncSession = Depends(get_db)):
