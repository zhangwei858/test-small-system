...
@router.get("")
async def get_prizes(db: AsyncSession = Depends(get_db)):
...
@router.post("")
async def create_prize(data: PrizeCreate, db: AsyncSession = Depends(get_db)):
...
@router.put("/{prize_id}")
async def update_prize(prize_id: int, data: PrizeUpdate, db: AsyncSession = Depends(get_db)):
...
async def delete_prize(prize_id: int, db: AsyncSession = Depends(get_db)):
...
@router.get("/dynamic")
async def get_dynamic_probabilities(question_count: int = 10, db: AsyncSession = Depends(get_db)):
