...
async def get_types(db = Depends(get_db)):
    ...
    return ApiResponse(data=[{"id": t["id"], "name": t["name"], "description": t.get("description")} for t in types])


@router.get("/{type_id}")
async def get_type(type_id: int, db = Depends(get_db)):
    t = await type_service.find_by_id(db, type_id)
    if not t:
        return ApiResponse(code=404, message="题型不存在")
    return ApiResponse(data={"id": t["id"], "name": t["name"], "description": t.get("description")})