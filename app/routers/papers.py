...
@router.get("")
async def get_papers(user_name: str = None, db = Depends(get_db)):
    papers = await paper_service.get_all_papers(db, user_name)
    return ApiResponse(data=papers)


@router.get("/{paper_id}")
async def get_paper(paper_id: int, db = Depends(get_db)):
    ...
    return ApiResponse(data={
        ...
        "created_at": paper["created_at"].isoformat() if paper.get("created_at") else None,
    })