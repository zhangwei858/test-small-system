...
async function startExamPreparation() {
  ...
  if (agid) {
    try {
      const [papers,types]=await Promise.all([api.getPapers(window.state.userName),api.getTypes()]);
...
function renderPaperCards(papers) {
  document.getElementById('paper-cards').innerHTML=papers.map(p=>`
    <div class="paper-card" onclick="startExamFromPaper(${window.state.selectedGradeId},${p.id})">
      ...
      <div class="paper-progress">已做 <span class="paper-done">${p.done_count||0}</span> 题 | 未做 <span class="paper-remain">${p.remaining_count||p.question_count}</span> 题</div>
    </div>
  `).join('');
}
