...
async function loadAllHistory() {
  try {
    ...
    const data = await api.getAllExamHistory(userName);
    renderHistoryList(data || []);
  } catch (e) { console.error('加载考试历史失败:', e); showEmptyState(); }
}

function toggleHistorySearch() {
  const searchBox = document.querySelector('.history-search');
  const userName = window.state?.userName || '';
  if (searchBox) {
    searchBox.style.display = userName === '张伟' ? '' : 'none';
  }
}
...
function goHome() {
  if (window.state.exam && window.state.exam.id) {
    navigateTo('exam');
  } else {
    navigateTo('home');
  }
...
function navigateTo(page) {
  ...
  else if (page === 'history') { loadAllHistory(); toggleHistorySearch(); }
